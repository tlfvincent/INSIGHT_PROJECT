...
def FindMoviesWithMetadata():
	# open new database to store IMDB and RT ratings data
	temp_df = pd.DataFrame.from_csv('tmdb_metadata.tab', sep='\t', header=0, index_col=False)
	# drop NAs in dataframe
	tmdb_metadata = temp_df.dropna(subset=['budget', 'revenue', 'year', 'runtime', 'genre'])
	# find remaining indices
	valid_index = list(tmdb_metadata.index.values)
	return(valid_index)
...
def MovieSentimentCrawler():
	...
	# find movies with available metadata in TMDB
	valid_index = FindMoviesWithMetadata()
